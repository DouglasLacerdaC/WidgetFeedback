...
export default {
  ...
  computed: {
    ...mapState(['listTypesFeedback', 'typeChosenUser', 'feedbackUser']),
    typeFeedback() {
      return this.listTypesFeedback[this.typeChosenUser]
    }
  },
  methods: {
    ...mapMutations(['CHANGE_TYPE_FEEDBACK', 'INSERT_FEEDBACK']),
    newFeedback() {
      ...
      this.INSERT_FEEDBACK({
        feedback
      })

      localStorage.setItem('feedback', JSON.stringify(this.feedbackUser))
...
<template>
  ...
  <div v-if="loading" class="flex flex-col justify-center items-center p-8">
    ...
    <span class="text-white text-center">Salvando seu feedback no localStorage...</span>
  </div>
  ...
  <div v-if="!loading" class="px-5 pb-4">
    <form>
      <textarea
        ...
        :placeholder="typeFeedback.placeholder" v-model="message"></textarea>
